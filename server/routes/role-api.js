        ...
        .exec(function(err, roles) {
         ...
         // Server Responses
          if (err) {
              ...
              console.log(roles);
              const findAllRolesBaseResponse = new BaseResponse('200', 'Query successful', roles);
  ...
  try {
      Role.findOne({ 'username': req.params.userName }, function(err, role) {
        ...
        // Server Responses
          if (err) {
              console.log(err);
              const findRoleByUserNameErrorResponse = new ErrorResponse('500', 'internal Server error', err);
              res.status(500).send(findRoleByUserNameErrorResponse.toObject());
          }
          else {
              console.log(role);
              const findRoleByUserNameBaseResponse = new BaseResponse('200', 'Query successful', role);
              res.json(findRoleByUserNameBaseResponse.toObject());
          }
      })
  } catch (e) {
      console.log(e);
      const findRoleByUserNameCatchErrorResponse = new ErrorResponse('500', 'Internal server error', e.message);
...
router.delete("/:roleId", async (req, res) => {
  try {
      ...
      Role.findOne({ _id: req.params.roleId }, function (err, role) {
          if (err) {
              console.log(err);
              const deleteRoleErrorResponse = new ErrorResponse("500", "Internal server error", err);
           ...
           // If successful
            ...
            // Look up role to determine if assigned to an existing user.
              ...
              function (err, users) {
                      ...
                      if (err) {
                          console.log(err);
                          const usersErrorResponse = new ErrorResponse("500", "Internal server error", err);
                          ...
                          if (users.length > 0) {
                              ...
                              role.save(function (err, updatedRole) {
                                  if (err) {
                                      console.log(err);
                                      const updatedRoleErrorResponse = new ErrorResponse("500", "Internal server error", err);
